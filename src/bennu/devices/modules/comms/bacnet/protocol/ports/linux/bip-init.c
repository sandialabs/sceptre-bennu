...
#include <ifaddrs.h>    /* for ifaddrs struct */
#include <string.h>
...
//    const char* addr = getenv("BACNET_IP_ADDR");
//    inet_pton(AF_INET, addr, &(sin.sin_addr));

//    const char* type = NULL;
//    type = getenv("BACNET_DEV_TYPE");
//    if (type && strcmp(type, "server") == 0)
//        sin.sin_addr.s_addr = bip_get_addr();
//    else
...
/** Gets the local interface name of a specified IP address
 * @param ip_addr [in] The IP address in dotted form (string)
 * @return the interface name as a string, or NULL
 */
char* get_ifname(const char *ip_addr)
{
    ...
    for (iap = addrs; iap != NULL; iap = iap->ifa_next) {
        if (iap->ifa_addr && (iap->ifa_flags & IFF_UP) && iap->ifa_addr->sa_family == AF_INET) {
            ...
            if (!strcmp(ip_addr, buf)) {
                if_name = iap->ifa_name;
            }
        }
    }
    freeifaddrs(addrs);
